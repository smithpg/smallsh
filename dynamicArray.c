 ...
 * Date: 7/31/17
    ...
    This file contains implementations for a number of
    interfaces over a dynamic array.
...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    if(rightIndex < last)
    {
        //Find min child ...
        int minIndex;
        if (compare(heap->data[rightIndex], heap->data[leftIndex]) >= 0)
        {
            minIndex = leftIndex;
        }
        else minIndex = rightIndex;

        //Swap and call self if necessary
        if(compare(heap->data[minIndex],heap->data[position]) < 0)
        {
    ...
    {
        //Swap and call self if necessary
        if(compare(heap->data[leftIndex],heap->data[position]) < 0)
        {
...
{
    int nonLeafIdx = heap->size/2 - 1;
    int i;
    for(i = nonLeafIdx; i >= 0; i--)
    {
...
{
    //Add new value to leftmost open node
    dyAdd(heap, value);

    //Percolate up
    int newValIdx = heap->size - 1;
    while((*compare)(heap->data[newValIdx],heap->data[(newValIdx-1)/2]) < 0)
    {
        dySwap(heap, newValIdx, (newValIdx-1)/2);
        newValIdx = (newValIdx-1)/2;
    }
}
